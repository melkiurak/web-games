...
export const GameCard = () => {
    return <div className="flex justify-between max-lg:gap-[16px] max-lg:whitespace-nowrap max-md:whitespace-nowrap max-lg:overflow-x-auto overflow-hidden ">
        {MOCK_GAME_CARDS.map((g, index) => (
            <div key={g.id} className="flex flex-col gap-3 border-1 border-solid border-[#7D3C98] p-[10px] max-desktop:p-2 rounded-xl" >
                <div className="w-[207.2px] max-desktop:w-[151.2px] h-[239px] max-desktop:h-[184px] max-lg:h-[178px] bg-center bg-cover bg-no-repeat rounded-xl" style={{backgroundImage: `url('${g.cover_url}')`}}>
                </div>
                <div className="flex flex-col gap-3">
                    ...
                    <div className="flex justify-between">
                        <div className="flex items-center gap-1">
                            ...
                            <span className="text-[#979797] text-xs font-light">{new Date(g.first_release_date).toLocaleDateString("en-US", {
                                year: 'numeric',
                            })}</span>
                        </div>
                        <div className="max-w-[97.6px] max-desktop:max-w-[75.6px] w-full h-full flex gap-1 items-center ">
                            ...
                            <span className="text-[#FFCC00] text-base font-medium">{g.rating}</span>  
                    ...
                    <div className="flex justify-between max-desktop:flex-col max-desktop:items-center max-desktop:gap-2">
                        <div className="w-[85px] flex justify-between items-center">
                            <span className={`${/*discountsPrice ? 'line-through text-[#979797] text-xs font-light' :*/ 'text-white font-main '}`}>{Math.round(g.price)}$</span>
